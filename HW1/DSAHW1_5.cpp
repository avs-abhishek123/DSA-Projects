...
{
	LARGE_INTEGER f, fb, fb2;
	int low, high, temp, pair_counter = 0, value1;
	//sort
	sort(data.begin(), data.end());
	//get time
	QueryPerformanceFrequency(&f);//get CPU frequency
	double t_duration = 1.0 / f.QuadPart;
	QueryPerformanceCounter(&fb);//tick quantity so far
    //real calculation
	...
	QueryPerformanceCounter(&fb2);//tick quantity so far
	return (fb2.QuadPart - fb.QuadPart)*t_duration * 1000;//parameter: ms
}
